...
from rcg.util.pos_embed import get_2d_sincos_pos_embed

from rcg.pixel_generator.mage.taming.models.vqgan import VQModel
from omegaconf import OmegaConf
import numpy as np
import scipy.stats as stats
import math
import rcg.pretrained_enc.models_pretrained_enc as models_pretrained_enc
from rcg.rdm.models.diffusion.ddim import DDIMSampler
from rcg.rdm.util import load_model
...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    def forward_encoder(self, x, rep, class_label):
        ...
        input_embeddings_after_drop = input_embeddings.reshape(bsz, -1, emb_dim)
    ...
    def forward_loss(self, gt_indices, logits):
        bsz, seq_len = gt_indices.size()
        # logits and mask are with seq_len+1 but gt_indices is with seq_len
        loss = self.criterion(logits[:, 1:, :self.codebook_size].reshape(bsz*seq_len, -1), gt_indices.reshape(bsz*seq_len))
        loss = loss.reshape(bsz, seq_len)
    ...
    def forward(self, imgs, class_label=None, gt=None, 
                gen_image=False, bsz=1, num_iter=20, choice_temperature=11.0,
                sampled_rep=None, rdm_steps=250, eta=1.0, cfg=0.0, class_label_gen=None):
        
        
        if gen_image:
            return self.gen_image(bsz, imgs, num_iter, choice_temperature, sampled_rep, rdm_steps, eta, cfg, class_label_gen)
        ...
        latent, gt_indices = self.forward_encoder(imgs, rep, class_label)
        gt_indices = self.get_gt(gt)
        logits = self.forward_decoder(latent)
        loss = self.forward_loss(gt_indices, logits)
        return loss

    def gen_image(self, bsz, img1, num_iter=12, choice_temperature=4.5, sampled_rep=None, rdm_steps=250, eta=1.0,
        ...
        initial_token_indices = self.get_token_indices(img1)
